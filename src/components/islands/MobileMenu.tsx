...
export default function MobileMenu({ initialProfile, currentPath = '/', labels }: MobileMenuProps) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Silent fail - data just won't show
            }
        };
    ...
    const specials = [
        { name: labels?.limited || 'Limited Drops', href: '/category/limited-drops', isFavorites: false },
        { name: labels?.discounts || 'Discounts', href: '/category/discounts', isFavorites: false },
        { name: labels?.coming || 'Coming soon', href: '/category/coming-soon', isFavorites: false },
        { name: labels?.viral || 'VIRAL TRENDS', href: '/category/viral-trends', isFavorites: false },
        { name: labels?.bestsellers || 'Bestsellers', href: '/category/bestsellers', isFavorites: false },
        { name: labels?.favorites || 'MY FAVORITES', href: '/category/my-favorites', isFavorites: true },
    ];
    ...
    return (
        <>
            ...
            {isOpen && (
                <div
                    className="fixed inset-0 bg-black/60 z-[95] transition-opacity backdrop-blur-md"
            ...
            {/* Slide-over panel */}
            <div
                className={`fixed inset-y-0 left-0 w-80 bg-white z-[100] shadow-2xl transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'
                    }`}
            >
                            ...
                            <a href="/category/all"
                                ...
                                <div className="relative z-10 pl-5">
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className="text-[6.5px] font-mono text-zinc-500 uppercase tracking-[0.3em] group-hover:text-white transition-colors">{labels?.allAccess || 'CATALOG_FULL_ACCESS'}</span>
                        ...
                        <div className="px-6 mb-8 pt-4">
                            ...
                            >
                                <div className="flex items-center gap-3 border-b border-black pb-4 w-full relative">
                                    ...
                                    <h3 className="font-black text-[14px] uppercase text-zinc-900 tracking-[0.1em] leading-none">{labels?.categories || 'Categories'}</h3>
                            ...
                            {expandedSection === 'categories' && (
                                <div className="space-y-6 animate-fade-in pb-8">
                                    ...
                                    {categories.length === 0 && <p className="text-xs text-zinc-400 pl-8">{labels?.categoriesEmpty || 'No categories found'}</p>}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="px-6 mb-8">
                            ...
                            >
                                <div className="flex items-center gap-3 border-b border-black pb-4 w-full relative">
                                    ...
                                    <h3 className="font-black text-[14px] uppercase text-zinc-900 tracking-[0.1em] leading-none">{labels?.brands || 'Brands'}</h3>
                            ...
                            {expandedSection === 'brands' && (
                                <div className="space-y-6 animate-fade-in pb-8">
                                    ...
                                    {brands.length === 0 && <p className="text-xs text-zinc-400 pl-8">{labels?.brandsEmpty || 'No brands found'}</p>}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="px-6 mb-14">
                            ...
                            >
                                <div className="flex items-center gap-3 border-b border-black pb-4 w-full relative">
                                    ...
                                    <h3 className="font-black text-[14px] uppercase text-zinc-900 tracking-[0.1em] leading-none">{labels?.colors || 'Cromatografía'}</h3>
                            ...
                            {expandedSection === 'colors' && (
                                <div className="space-y-4 animate-fade-in pb-4 px-1">
                                    ...
                                    {colors.length === 0 && <p className="text-xs text-zinc-400 pl-8">{labels?.colorsEmpty || 'No colors found'}</p>}
                                </div>
                            )}
                        </div>
                            ...
                            <div className="space-y-3">
                                ...
                                >
                                    <div className="flex flex-col items-start gap-0.5 z-10">
                                        <span className="text-zinc-300 text-[9px]">{labels?.account || 'ACCOUNT'}</span>
                                        <span className="text-zinc-900 text-[12px] leading-tight">{labels?.profile || 'USUARIO / PROFILE'}</span>
                                    </div>
                                ...
                                {isAdmin && (
                                    ...
                                    >
                                        <div className="flex flex-col items-start gap-0.5 z-10">
                                            <span className="text-zinc-500 text-[9px]">{labels?.admin || 'ADMIN'}</span>
                                            <span className="text-white text-[12px] leading-tight">{labels?.adminConsole || 'CONTROL CONSOLE'}</span>
                                        </div>
                        ...
                        <div className="px-6 mb-14 py-6 border-t border-zinc-100 bg-zinc-50/50">
                            <div className="w-full text-left mb-6">
                                <div className="flex items-center gap-3 border-b border-black pb-4 w-full relative">
                                    ...
                                    <h3 className="font-black text-[14px] uppercase text-zinc-900 tracking-[0.1em] leading-none">{labels?.help || 'Help & Contact'}</h3>
                                </div>
                            </div>
                            <div className="space-y-4 animate-fade-in pb-4 px-1">
                                {[
                                    { name: labels?.service || 'CUSTOMER SERVICE / CONTACT', slug: 'customer-service' },
                                    { name: labels?.size || 'SIZE GUIDE', slug: 'size-guide' },
                                    { name: labels?.returns || 'RETURNS', slug: 'returns' },
                                    { name: labels?.faq || 'FAQ', slug: 'faq' },
                                    { name: labels?.about || 'ABOUT US', slug: 'about' },
                                    { name: labels?.shipping || 'SHIPPING & RETURNS', slug: 'shipping' }
                                ].map((item) => (
                                    <a
                                        key={item.name}
                                        href={`/${item.slug}`}
